...
-- `drop policy if exists` before each `create policy` -- CREATE POLICY has no IF NOT EXISTS in
-- Postgres, so without this a second run of this file fails with "policy already exists"
...
-- (Superseded by supabase_accounts_schema.sql's scores_anon_insert once that file runs -- kept
-- idempotent here too since this file can still be re-run on its own.)
